            ...
            <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 card-soberana rounded-2xl p-8">
                    ...
                    <div className="mt-10 border-t border-slate-800 pt-8">
                        ...
                        <div className="flex items-center gap-3"><button type="button" onClick={() => setExtraPages(Math.max(0, extraPages - 1))} className="w-10 h-10 bg-black border border-slate-700 rounded-lg text-white">−</button><span className="w-16 text-center text-white font-bold">{extraPages}</span><button type="button" onClick={() => setExtraPages(extraPages + 1)} className="w-10 h-10 bg-black border border-slate-700 rounded-lg text-white">+</button></div>
                    </div>

                    <div className="mt-10 border-t border-slate-800 pt-8">
                        <h2 className="text-2xl font-bold mb-4 text-white font-syne">3. Servicio mensual opcional</h2>
                        <p className="text-slate-400 text-sm mb-4">No se suma a la inversión inicial. Se muestra aparte para evitar mezclar pagos únicos y cuotas.</p>
                        <select value={maintenancePlan} onChange={event => setMaintenancePlan(event.target.value as MaintenanceKey)} className="w-full bg-black/30 border border-slate-700 rounded-xl p-3 text-white">
